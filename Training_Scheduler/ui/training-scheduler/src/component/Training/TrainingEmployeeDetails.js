...
import { fetchAllEmployee, fetchTrainingDetails, fetchTrainingRegisteredEmployee, saveTrainingMapperEmployee } from "../Api/ApiManager";

function TrainingEmployeeDetails() {
  ...
  useEffect(()=>{
    fetchTrainingRegisteredEmployee(id).
    then((data)=>{
      console.log(data)
      createRowData(data)
    })
  ...
  return (
    <>
          ...
          <Button className="float-start createButton" onClick={addUser}>
            Remove Selected Employees from Training
          </Button>
        <div
...
export default TrainingEmployeeDetails;
